...
LD_plot <- function(df, ld_df = NULL, lead_snps = NULL,
  ...
  if (!is.null(bfile) && file.exists(paste0(bfile, ".bed"))) {
    ...
    if (!is.null(ld_result)) {
      ...
      if (nrow(ld_to_lead) > 0) {
        # Use abs() to ensure r² is always positive (r can be negative but r² cannot)
  ...
  # Lead SNP r²=1, background=0.05 (lighter for better contrast)
  ...
  # Professional color scheme (standard LD heatmap colors)
  ...
  # Base plot with recombination rate as background
  p <- ggplot()
  ...
  p <- p +
    ...
    # Color scale with publication-quality legend
    scale_fill_identity(
      ...
      guide = guide_legend(
        override.aes = list(shape = 21, size = 5, alpha = 1, stroke = 0.5, color = "white"),
        title.position = "top",
        title.hjust = 0.5
      )
    ) +
...
# ==============================================================================
# Gene Track (Publication-Quality with Strand Arrows)
# ==============================================================================
genetrack <- function(chrom_str, xlim_bp, gtf_path = NULL) {
    ...
    if (!is.null(gtf_path) && file.exists(gtf_path)) {
        tryCatch({
            ...
            if (length(subset_gtf) > 0) {
                ...
                # Get strand info - FIX: ensure it's a simple character vector
                ...
                if (nrow(df_gtf) > 0) {
                    # FIX: Use dplyr::first() explicitly and ensure proper grouping
                    tx_summary <- df_gtf %>%
    ...
    if (is.null(tx_df) || nrow(tx_df) == 0) {
        tryCatch({
            ...
            if (length(subset_tx) > 0) {
                # FIX: Check if gene_id exists and has valid entries
                ...
                # Create data frame with fallback names
                tx_df <- data.frame(
    ...
    # Fix strand notation - ensure valid values
    tx_df$strand[is.na(tx_df$strand) | tx_df$strand == "*" | tx_df$strand == ""] <- "+"
    # Ensure only + or - values
    tx_df$strand <- ifelse(tx_df$strand %in% c("+", "-"), tx_df$strand, "+")

    # Intelligent staggered layout (avoid overlaps)
    tx_df$y <- 1
    if (nrow(tx_df) > 1) {
        for (i in 2:nrow(tx_df)) {
            overlap_margin <- (BPStop - BPStart) * 0.03  # 3% margin
    ...
    # Strand-based colors
    tx_df$color <- ifelse(tx_df$strand == "+", "#D73027", "#4575B4")  # Red=forward, Blue=reverse

    # Create base plot
    p <- ggplot(tx_df)
    
    # Gene body (thick line)
    p <- p + geom_segment(
    ...
    arrow_length <- (BPStop - BPStart) * 0.015  # 1.5% of window
    ...
    p <- p +
      ...
      theme(
        ...
        axis.ticks.length.x = unit(0.15, "cm"),
        axis.line.x = element_line(color = "black", linewidth = 0.6),
        legend.position = "none"  # Can enable if desired
      )
...
plot_qtl_association <- function(qtl_all_chrom, qtl_all_pvalue, leadSNP_DF,
    ...
    # Publication-quality layout with proper spacing
